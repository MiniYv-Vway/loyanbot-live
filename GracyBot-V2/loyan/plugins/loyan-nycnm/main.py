...
CACHE_DIR = os.path.join(get_storage_dir(), "data", "nycnm")
TIMEOUT = 20
...
async def handle_ai(ctx: PluginContext):
    ...
    await ctx.reply("🤖 AI 思考中...")
    try:
        # 1. 调用框架 AI（Ollama/提供商）
        from loyan.brain import get_brain
        brain = get_brain()
        reply = await brain.chat.chat(question, session_id=str(getattr(ctx, "sender_id", "")))
        reply = (reply or "").strip()
        if not reply or reply.startswith(" "):
            await ctx.reply(f"🤖 AI 暂不可用：{reply or '无回复'}")
            return

        # 2. 用恶搞语音朗读 AI 回复
        url = _build_url("/kktts", {"type": "mp3", "uid": 1, "text": reply[:100]})
        result = await asyncio.to_thread(_request_json, url)
        if result and result.get("code") == 200:
            ...
            if audio_url:
                try:
                    ...
                    logger.warning(f"AI语音下载失败: {dl_err}")

        # 3. 同时发文字回复
        await ctx.reply(f"🤖 {reply[:500]}")
    except Exception as e:
        logger.error(f"AI 对话失败: {e}")
        await ctx.reply("❌ AI 对话失败，请稍后再试")
