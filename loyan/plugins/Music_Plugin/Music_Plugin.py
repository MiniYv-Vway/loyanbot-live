...
from graci import on_command, plugin_handler, PluginContext, get_storage_dir
...
def _search_key(chat_type: str, target_id) -> str:
    return f"{chat_type}:{target_id}"


DATA_DIR = os.path.join(get_storage_dir(), "data", "loyan-music")
...
async def handle_music(ctx: PluginContext):
    """音乐点歌 — 搜索 / 播放 / 详情 / 清理缓存"""
    skey = _search_key(ctx.chat_type, ctx.target_id or ctx.sender_id)
    ...
    if cmd in ("/点歌", "/qq音乐"):
        ...
        cached = _last_search[skey]

        try:
            img_path = await asyncio.to_thread(draw_search_results, cached["results"], args)
        except Exception as e:
            logger.error(f"[点歌] 绘图异常: {e}", exc_info=True)
            lines = [f"🎵 搜索: {args}\n"]
            for i, s in enumerate(cached["results"]):
                lines.append(f"  {i+1}. {s.name} — {s.artist}")
            lines.append(f"\n💡 发送 /选择 编号 查看详情 (共{len(cached['results'])}首)")
            await ctx.reply("\n".join(lines))
            return

        await ctx.send(LoyanImage(file_path=img_path))
        logger.info(f"用户{ctx.sender_id} 点歌搜索: {args} ({len(cached['results'])}首)")
        return

    # ── /选择 N — 选取播放 ──
    if cmd == "/选择":
        cached = _last_search.get(skey)
        if not cached or not cached["results"]:
        ...
        if idx < 1 or idx > len(results):
            await ctx.reply(f"❌ 编号超出范围（1-{len(results)}）")
            return
        ...
        try:
            detail = await play_song(cached["keyword"], selected.idx)
        except Exception as e:
        ...
        try:
            img_path = await asyncio.to_thread(draw_song_detail, detail)
        except Exception as e:
    ...
    # ── /清理缓存 — 清理过期缓存文件 ──
    if cmd == "/清理缓存":
        cfg = plugin_manager.get_plugin_config("Music_Plugin") or {}
            ...
            if os.path.isfile(fpath):
                ...
                if file_age_secs > max_days * 86400:
                    try:
                        ...
                        logger.warning(f"[清理缓存] 删除失败 {fpath}: {e}")
                else:
                    kept += 1
    ...
    if cmd == "/歌曲详情":
        cached = _last_search.get(skey)
        selected = (cached or {}).get("selected")
        if selected is None:
            await ctx.reply("⚠️ 还没有选过歌曲，请先 /点歌 搜索后再 /选择")
            return

        try:
            img_path = await asyncio.to_thread(draw_song_detail, selected)
        except Exception as e:
            logger.error(f"[详情] 绘图异常: {e}", exc_info=True)
            await ctx.reply(f"🎵 {selected.name}\n歌手: {selected.artist}")
            return
